...
export async function buildWorkerScopeWhere(user: JwtClaims): Promise<Record<string, any>> {
  ...
  if (roleId === 4) {
    ...
    const [recruitLinks, agencyEmployerLinks] = await Promise.all([
      prisma.tbl_Worker_RecruitAgent.findMany({
        where: {
      ...
      prisma.tbl_Agency_Employer_Link.findMany({
    ...
    for (const r of recruitLinks ?? []) {
      ...
      if (v) workerIdSet.add(v);
    }

    const employerIds = Array.from(
      new Set((agencyEmployerLinks ?? []).map((x) => (x.employerId ?? "").toString().trim()).filter(Boolean))
    );

    // Build OR clauses: workers by ID set + workers under any linked employer.
    const ors: Array<Record<string, any>> = [];
    if (workerIdSet.size > 0) ors.push({ Worker_Id: { in: Array.from(workerIdSet) } });
    if (employerIds.length > 0) ors.push({ Employer_Id: { in: employerIds } });

    if (ors.length === 0) return { Worker_Id: "" };
