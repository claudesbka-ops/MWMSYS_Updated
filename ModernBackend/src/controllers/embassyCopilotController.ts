...
import { processEmbassyQuery } from "../services/embassyCopilotService";
...
export async function queryEmbassyCopilot(req: Request, res: Response) {
  try {
    ...
    const result = await processEmbassyQuery(message, history, user);
    ...
    console.error("[embassy-copilot] Controller error:", error);
    res.status(500).json({
      success: false,
