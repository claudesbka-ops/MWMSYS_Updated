...
import { broadcastUpload } from "../middleware/upload";
import { broadcastTableExists, ensureBroadcastTableExists } from "../db/schemaMigrations";
...
broadcastRouter.post("/Api/Broadcast/Send", requireAuth, async (req, res, next) => {
  try {
...
broadcastRouter.get("/Api/Broadcast/Feed", requireAuth, async (req, res, next) => {
  try {
    await ensureBroadcastTableExists();
    const ok = await broadcastTableExists();
    if (!ok) return res.status(501).json({ error: "Broadcast table not installed" });
