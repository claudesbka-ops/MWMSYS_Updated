...
subscriptionRouter.get("/stripe/return", (req, res) => {
...
subscriptionRouter.post("/Api/subscription/checkout", requireAuth, async (req, res, next) => {
  try {
...
subscriptionRouter.post("/webhooks/stripe", async (req, res) => {
  try {
...
subscriptionRouter.get("/Api/subscription/me", requireAuth, async (req, res, next) => {
  try {
...
subscriptionRouter.post("/Api/subscription/purchase", requireAuth, async (req, res, next) => {
  try {
    const user = (req as any).user as { userKey?: string; roleId?: number };
    const roleId = user?.roleId != null ? Number(user.roleId) : null;
    if (roleId !== 3 && roleId !== 4) {
