...
const disputesDir = path_1.default.join(upload_1.uploadsDir, "disputes");
fs_1.default.mkdirSync(disputesDir, { recursive: true });
...
const disputeUpload = (0, multer_1.default)({
    ...
    fileFilter: (_req, file, cb) => {
        if (!file)
            return cb(null, true);
        if (ALLOWED_PROOF_MIME.has((file.mimetype ?? "").toLowerCase()))
            return cb(null, true);
        cb(new Error("Proof must be an image or PDF"));
    },
});
exports.disputeRouter = (0, express_1.Router)();
async function hydrateDisputes(rows) {
    if (!rows || rows.length === 0)
        return [];
    const workerIds = Array.from(new Set(rows.map((r) => (r.Worker_Id ?? "").toString()).filter(Boolean)));
    const employerIds = Array.from(new Set(rows.map((r) => (r.Employer_Id ?? "").toString()).filter(Boolean)));
    const [workers, employers] = await Promise.all([
        workerIds.length
            ? db_1.prisma.tbl_Worker_PersonalInfo.findMany({
    ...
    const employerMap = new Map((employers ?? []).map((e) => [(e.User_Id ?? "").toString(), (e.Employer_Name ?? "").toString()]));
    return rows.map((r) => ({
        id: r.Id,
...
exports.disputeRouter.post("/Api/Dispute/Submit", auth_1.requireAuth, disputeUpload.single("proof"), async (req, res, next) => {
    try {
        const user = req.user;
        const roleId = Number(user?.roleId ?? 0);
        if (roleId !== 2) {
            if (req.file)
                safeUnlinkProof(req.file.filename);
        ...
        await (0, schemaMigrations_1.ensureDisputeTableExists)();
        // Resolve employer from the worker record.
        const workerRow = await db_1.prisma.tbl_Worker_PersonalInfo.findFirst({
        ...
        const created = await db_1.prisma.tbl_SalaryDispute.create({
            data: {
                Worker_Id: workerId,
        ...
        const hydrated = await hydrateDisputes([created]);
...
function safeUnlinkProof(filename) {
    (0, upload_1.safeUnlinkUpload)(`/uploads/disputes/${filename}`);
}
// ---------- GET /Api/Dispute/MyDisputes (worker) ----------
exports.disputeRouter.get("/Api/Dispute/MyDisputes", auth_1.requireAuth, async (req, res, next) => {
    try {
        const user = req.user;
        const roleId = Number(user?.roleId ?? 0);
        if (roleId !== 2)
            return res.status(403).json({ error: "Only workers can view their disputes" });
        const workerId = (user?.userKey ?? "").toString().trim();
        if (!workerId)
            return res.json([]);
        await (0, schemaMigrations_1.ensureDisputeTableExists)();
        const rows = await db_1.prisma.tbl_SalaryDispute.findMany({
            where: { Worker_Id: workerId },
            orderBy: [{ Submitted_At: "desc" }],
            take: 500,
        });
        return res.json(await hydrateDisputes(rows));
    }
    catch (e) {
        return next(e);
    }
...
exports.disputeRouter.get("/Api/Dispute/Incoming", auth_1.requireAuth, async (req, res, next) => {
    try {
        const user = req.user;
        const roleId = Number(user?.roleId ?? 0);
        if (roleId !== 3)
            return res.status(403).json({ error: "Only employers have a dispute inbox" });
...
exports.disputeRouter.get("/Api/Dispute/All", auth_1.requireAuth, async (req, res, next) => {
    try {
        const user = req.user;
        const roleId = Number(user?.roleId ?? 0);
        ...
        if (roleId === 4) {
            ...
            const scoped = await db_1.prisma.tbl_Worker_PersonalInfo.findMany({
                where: scopeWhere,
                select: { Worker_Id: true },
                take: 5000,
            });
            workerIdFilter = Array.from(new Set((scoped ?? []).map((w) => (w.Worker_Id ?? "").toString()).filter(Boolean)));
            if (workerIdFilter.length === 0)
                return res.json([]);
        }
        const where = {};
        if (workerIdFilter)
            where.Worker_Id = { in: workerIdFilter };
...
exports.disputeRouter.put("/Api/Dispute/Review", auth_1.requireAuth, async (req, res, next) => {
    try {
        const user = req.user;
        const roleId = Number(user?.roleId ?? 0);
        ...
        await (0, schemaMigrations_1.ensureDisputeTableExists)();
        const existing = await db_1.prisma.tbl_SalaryDispute.findFirst({ where: { Id: id } });
        if (!existing)
        ...
        const updated = await db_1.prisma.tbl_SalaryDispute.update({
            where: { Id: id },
            data: {
                Status: status,
        ...
        const hydrated = await hydrateDisputes([updated]);
...
exports.disputeRouter.get("/Api/Dispute/File/:id", auth_1.requireAuth, async (req, res, next) => {
    try {
        ...
        await (0, schemaMigrations_1.ensureDisputeTableExists)();
        const dispute = await db_1.prisma.tbl_SalaryDispute.findFirst({ where: { Id: id } });
        ...
        if (roleId === 1 || roleId === 7) {
            ...
            const scopeWhere = await (0, queryGuard_1.buildWorkerScopeWhere)(user);
            const match = await db_1.prisma.tbl_Worker_PersonalInfo.findFirst({
        ...
        const filename = path_1.default.basename(proofPath);
        if (!filename || !proofPath.startsWith("/uploads/disputes/")) {
            return res.status(404).json({ error: "File missing" });
        }
        const full = path_1.default.join(disputesDir, filename);
