...
const documentValidity_1 = require("./services/documentValidity");
...
app.post("/Api/Worker/Location", auth_1.requireAuth, async (req, res, next) => {
    try {
        ...
        await db_1.prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "Tbl_Worker_Location" (
        ...
        await db_1.prisma.$executeRawUnsafe(`INSERT INTO "Tbl_Worker_Location"("workerId", lat, lng, accuracy, "updatedOn") VALUES ($1, $2, $3, $4, $5) ON CONFLICT ("workerId") DO UPDATE SET lat=EXCLUDED.lat, lng=EXCLUDED.lng, accuracy=EXCLUDED.accuracy, "updatedOn"=EXCLUDED."updatedOn"`, workerId, latNum, lngNum, accVal, updatedOn);
        const workerInfo = await db_1.prisma.tbl_Worker_PersonalInfo.findFirst({
            where: { Worker_Id: workerId },
            select: { Employer_Id: true, Nationality: true, Name: true },
        });
        const agencyLinks = await db_1.prisma.tbl_Worker_RecruitAgent.findMany({
            where: { Worker_Id: workerId },
        ...
        try {
            ...
            if (employerId)
                io.to(`employer:${employerId}`).emit("worker_location_update", payload);
            ...
            if (Number.isFinite(nationality))
                io.to(`nationality:${nationality}`).emit("worker_location_update", payload);
        }
        catch {
            // ignore emit errors
        }
...
app.get("/Api/HRMS/Timesheets", auth_1.requireAuth, async (req, res, next) => {
    try {
        const roleId = Number(req.user?.roleId ?? 0);
        ...
        const rosterRows = (await db_1.prisma.$queryRawUnsafe(roleId === 1
            ...
            : `SELECT r."workerId", r."date", s."startTime", s."endTime", s."breakMinutes" FROM "Tbl_Roster_Assignment" r LEFT JOIN "Tbl_Shift_Template" s ON s.id=r."shiftId" WHERE r."date" >= CAST($1 AS DATE) AND r."date" <= CAST($2 AS DATE) AND r."workerId" IN (${workerIds
                .map((_, i) => `$${i + 3}`)
                .join(",")}) LIMIT 50000`, fromStr, toStr, ...(roleId === 1 ? [] : workerIds)));
        const attendanceRows = (await db_1.prisma.$queryRawUnsafe(roleId === 1
        ...
        const plannedByWorkerDay = new Map();
        ...
        const actualByWorkerDay = new Map();
        for (const a of attendanceRows ?? []) {
            ...
            if (!workerId || !cin || isNaN(cin.getTime()))
                continue;
        ...
        for (const k of plannedByWorkerDay.keys())
            allKeys.add(k);
        for (const k of actualByWorkerDay.keys())
            allKeys.add(k);
...
app.get("/Api/Workers/Locations", auth_1.requireAuth, (0, auth_1.checkRole)([1, 3, 4, 5, 6, 7]), async (req, res, next) => {
    try {
        const roleId = Number(req.user?.roleId ?? 0);
        const scopeWhere = await (0, queryGuard_1.buildWorkerScopeWhere)(req.user);
        const scopedWorkers = await db_1.prisma.tbl_Worker_PersonalInfo.findMany({
        ...
        const locations = (await db_1.prisma.$queryRawUnsafe(roleId === 1
            ...
            : `SELECT "workerId", lat, lng, accuracy, "updatedOn" FROM "Tbl_Worker_Location" WHERE "workerId" IN (${workerIds
                .map((_, i) => `$${i + 1}`)
                .join(",")}) ORDER BY "updatedOn" DESC LIMIT 5000`, ...(roleId === 1 ? [] : workerIds)));
        const nameById = new Map();
        for (const w of scopedWorkers ?? []) {
            nameById.set((w.Worker_Id ?? "").toString(), w.Name ?? null);
        }
        return res.json({
            rows: (locations ?? []).map((l) => ({
                ...
                updatedAt: l.updatedOn ? new Date(l.updatedOn).toISOString() : null,
            })),
        });
    }
    catch (e) {
...
app.post("/Api/Attestation/Submit", auth_1.requireAuth, (0, auth_1.checkRole)([2]), upload_1.upload.single("file"), async (req, res, next) => {
    ...
    try {
        ...
        const validity = (0, documentValidity_1.evaluateDocument)(docType, {
...
app.get("/Api/Attestation/List", auth_1.requireAuth, (0, auth_1.checkRole)([1, 4, 5, 6, 7]), async (req, res, next) => {
    try {
        await (0, schemaMigrations_1.ensureAttestationTableExists)();
        const rows = (await db_1.prisma.$queryRawUnsafe(`SELECT "AttestationId", "Worker_Id", "Passport_Number", "DocumentType", "DocumentPath", "Status", "AdminRemarks", "Created_On", "Updated_On",
         ...
         LIMIT 500`));
        const callerRoleId = Number(req.user?.roleId ?? 0);
        ...
        const enriched = (rows ?? []).map((r) => {
            ...
            const validity = (0, documentValidity_1.evaluateDocument)(r.DocumentType, {
 ...
 *   - Anyone else:  403.
 */
app.get("/Api/Attestation/:id/Document", auth_1.requireAuth, async (req, res, next) => {
    try {
        await (0, schemaMigrations_1.ensureAttestationTableExists)();
        ...
        const callerRoleId = Number(req.user?.roleId ?? 0);
        const callerKey = (req.user?.userKey ?? "").toString().trim();
        const rows = (await db_1.prisma.$queryRawUnsafe(`SELECT "AttestationId", "Worker_Id", "DocumentPath", "Status" FROM "Tbl_Attestation" WHERE "AttestationId" = $1 LIMIT 1`, id));
        const row = rows?.[0];
        if (!row)
        ...
        if (callerRoleId === 1) {
            ...
            if (status === "approved") {
                const link = await db_1.prisma.tbl_Worker_PersonalInfo.findFirst({
        ...
        const fullPath = path.join(upload_1.uploadsDir, safeName);
        if (!fullPath.startsWith(upload_1.uploadsDir) || !fs.existsSync(fullPath)) {
...
app.post("/Api/Attestation/Approve", auth_1.requireAuth, (0, auth_1.checkRole)([1]), async (req, res, next) => {
    ...
    try {
        await (0, schemaMigrations_1.ensureAttestationTableExists)();
        await db_1.prisma.$executeRawUnsafe(`UPDATE "Tbl_Attestation" SET "Status" = 'Approved', "AdminRemarks" = $1, "Updated_On" = NOW() WHERE "AttestationId" = $2`, remarks, id);
        return res.json({ ok: true });
    }
    catch (e) {
        return next(e);
...
app.post("/Api/Attestation/Reject", auth_1.requireAuth, (0, auth_1.checkRole)([1]), async (req, res, next) => {
    ...
    try {
        await (0, schemaMigrations_1.ensureAttestationTableExists)();
        await db_1.prisma.$executeRawUnsafe(`UPDATE "Tbl_Attestation" SET "Status" = 'Rejected', "AdminRemarks" = $1, "Updated_On" = NOW() WHERE "AttestationId" = $2`, remarks, id);
        return res.json({ ok: true });
    }
    catch (e) {
        return next(e);
...
app.get("/Api/Worker/Documents", auth_1.requireAuth, async (req, res, next) => {
    try {
        ...
        await (0, schemaMigrations_1.ensureAttestationTableExists)();
        const attestations = (await db_1.prisma.$queryRawUnsafe(`SELECT "AttestationId", "DocumentType", "DocumentPath", "Status", "AdminRemarks", "Created_On", "Updated_On"
         FROM "Tbl_Attestation"
        WHERE "Worker_Id" = $1
        ORDER BY "AttestationId" DESC`, workerId));
        const latestAttestationByType = new Map();
        for (const a of attestations ?? []) {
            const key = (a.DocumentType ?? "").toString().trim().toLowerCase();
            if (key && !latestAttestationByType.has(key))
                latestAttestationByType.set(key, a);
        }
        const docs = [
...
app.post("/Api/Incidents/Approve", auth_1.requireAuth, auth_1.requireAdmin, async (req, res, next) => {
    try {
        ...
        if (!row)
            return res.status(404).json({ error: "Not found" });
        const updated = await db_1.prisma.tbl_ProbSol.update({
            where: { ID: id },
            data: { ProbStatus: "Approved", Updated_On: new Date(), Updated_By: (req.user?.userKey ?? "admin").toString() },
        });
        const payload = {
            id: updated.ID,
            title: (updated.Title ?? updated.Type ?? "Alert").toString(),
            description: (updated.Description ?? "").toString(),
            workerId: updated?.worker_ID ?? null,
            companyName: updated?.Company_Name ?? null,
            currentLocation: updated?.Current_Location ?? null,
            status: "Approved",
            createdAt: updated.Updated_On ? new Date(updated.Updated_On).toISOString() : new Date().toISOString(),
        };
        try {
...
app.get("/api/admin/stats", auth_1.requireAuth, auth_1.requireAuthority, async (_req, res, next) => {
    try {
        ...
        const recentUsers = (await db_1.prisma.$queryRawUnsafe(`SELECT "ID", "User_Id", "User_Name", "Created_On" FROM "Tbl_User" ORDER BY "Created_On" DESC, "ID" DESC LIMIT 5`));
        const activity = [];
...
app.get("/Api/Employers/List", auth_1.requireAuth, auth_1.requireAuthority, async (_req, res, next) => {
    try {
        const rows = (await db_1.prisma.$queryRawUnsafe(`SELECT "User_Id", "Employer_Name", "Employer_Address", "Employer_ContactPerson", "Employer_Position", "Employer_EmailID", "Employer_OfficeNumber", "Employer_PIC_MobileNumber", "Created_On" FROM "Tbl_Employer" ORDER BY "Created_On" DESC LIMIT 500`));
        return res.json(rows ?? []);
    }
    catch (e) {
        return next(e);
 ...
 */
app.get("/Api/Agencies/List", auth_1.requireAuth, auth_1.requireAuthority, async (_req, res, next) => {
    try {
        const rows = (await db_1.prisma.$queryRawUnsafe(`SELECT "User_Id", "Agent_Name", "Agent_Organization_Name", "Agent_IC_Passport", "Agent_EmailID", "Agent_ContactNumber", "Agent_Country", "Agent_CountryCode", "Created_On" FROM "Tbl_Agent" ORDER BY "Created_On" DESC LIMIT 500`));
        return res.json(rows ?? []);
    }
    catch (e) {
        return next(e);
...
app.get("/Api/HRMS/Roster/Shifts", auth_1.requireAuth, async (req, res, next) => {
    try {
        const roleId = Number(req.user?.roleId ?? 0);
        ...
        const rows = (await db_1.prisma.$queryRawUnsafe(roleId === 1
            ...
            : `SELECT id, "entityId", name, "startTime", "endTime", "breakMinutes" FROM "Tbl_Shift_Template" WHERE "entityId" = $1 ORDER BY id DESC LIMIT 500`, ...(roleId === 1 ? [] : [entityId])));
        return res.json((rows ?? []).map((r) => ({
            id: Number(r.id),
...
app.post("/Api/HRMS/Roster/Shifts", auth_1.requireAuth, subscription_1.requireActivePlanForWrite, async (req, res, next) => {
    try {
        ...
        const inserted = (await db_1.prisma.$queryRawUnsafe(`INSERT INTO "Tbl_Shift_Template"("entityId",name,"startTime","endTime","breakMinutes") VALUES($1,$2,$3,$4,$5) RETURNING id`, entityId || "admin", name, startTime, endTime, Number.isFinite(breakMinutes) ? breakMinutes : 0));
...
app.post("/Api/HRMS/Roster/Assign", auth_1.requireAuth, subscription_1.requireActivePlanForWrite, async (req, res, next) => {
    try {
        ...
        await db_1.prisma.$executeRawUnsafe(`INSERT INTO "Tbl_Roster_Assignment"("workerId", "entityId", "date", "shiftId") VALUES ($1, $2, CAST($3 AS DATE), $4) ON CONFLICT ("workerId", "date") DO UPDATE SET "shiftId"=EXCLUDED."shiftId", "entityId"=EXCLUDED."entityId", "updatedOn"=NOW()`, workerId, entityId || "admin", dStr, shiftId);
        return res.json({ ok: true });
    }
    catch (e) {
        return next(e);
...
app.get("/Api/HRMS/Roster", auth_1.requireAuth, async (req, res, next) => {
    try {
        const roleId = Number(req.user?.roleId ?? 0);
        if (![1, 3, 4, 5, 6, 7].includes(roleId))
            return res.status(403).json({ error: "Forbidden" });
        ...
        const rows = (await db_1.prisma.$queryRawUnsafe(roleId === 1
            ...
            : `SELECT r.id, r."workerId", r."date", r."shiftId", s.name AS "shiftName", s."startTime", s."endTime", s."breakMinutes" FROM "Tbl_Roster_Assignment" r LEFT JOIN "Tbl_Shift_Template" s ON s.id=r."shiftId" WHERE r."date" >= CAST($1 AS DATE) AND r."date" <= CAST($2 AS DATE) AND r."workerId" IN (${workerIds
                .map((_, i) => `$${i + 3}`)
                .join(",")}) ORDER BY r."date" DESC, r.id DESC LIMIT 5000`, fromStr, toStr, ...(roleId === 1 ? [] : workerIds)));
        const parseTime = (t) => {
            const m = /^\s*(\d{1,2}):(\d{2})\s*$/.exec(String(t ?? ""));
            if (!m)
                return null;
...
app.get("/Api/Incidents/:id", auth_1.requireAuth, auth_1.requireAlertViewer, async (req, res, next) => {
    ...
    try {
        ...
        if (!row)
            return res.status(404).json({ error: "Not found" });
        if (roleId !== 1 && (row?.ProbStatus ?? "").toString() === "Pending") {
            return res.status(403).json({ error: "Forbidden" });
        }
        if (roleId !== 1) {
            const scopeWhere = await (0, queryGuard_1.buildWorkerScopeWhere)(req.user);
